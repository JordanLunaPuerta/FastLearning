...
namespace FastLearning.Controllers
{
    public class DescuentoController : Controller
    {
        ...
        public ActionResult DescuentoReportes(/*string buscar*/)
        {
            //List<Descuento> descuentos = db.Descuento.Where(d => d.estado == 1).ToList();
            //if (!String.IsNullOrEmpty(buscar))
            //{
            //    descuentos = descuentos.Where(d => d.nombre.Contains(buscar)).ToList();
            //}
            DateTime fecha_min = new DateTime(2018 - 04 - 30);
            DateTime fecha_max = new DateTime(2018 - 06 - 13);

            return View(db.spDescuentoUsuarioPorFecha(fecha_min:fecha_min,fecha_max:fecha_max));
        }
